...
const reminderInputSchema = z.object({
  time: z
    .string()
    .describe('Duration from now when the reminder should occur (e.g., "10s", "5m", "2h", "1d").'),
  message: z.string().describe('The content of the reminder message.'),
});

type ReminderInput = z.infer<typeof reminderInputSchema>;

interface ReminderExecuteOptions {
  experimental_context?: unknown;
}

async function executeReminderTool(args: ReminderInput, options?: ReminderExecuteOptions) {
