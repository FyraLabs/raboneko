...
client.on(Events.MessageUpdate, async (oldMessage, newMessage) => {
  if (!oldMessage.member || oldMessage.author?.bot || newMessage.author?.bot) {
    return;
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .setFooter({
      text:
        `Message ID: ${oldMessage.id} | User ID: ${oldMessage.member.user.id}`,
    }).data;
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields({
      ...
      value: member.joinedAt
        ? time(member.joinedAt, TimestampStyles.ShortDateTime)
        : "Unknown",
    })
    .setFooter({
      text: `Member ID: ${member.id} | User ID: ${member.user.id}`,
