...
const onnxRuntimeWebRoot = path.dirname(
  require.resolve("onnxruntime-web/package.json"),
);
const vadOnnxRuntimeWebRoot = path.dirname(
  require.resolve(
    "@ricky0123/vad-web/node_modules/onnxruntime-web/package.json",
  ),
);
const vadModelPath = require.resolve(
  "@ricky0123/vad-web/dist/silero_vad_legacy.onnx",
);

Settings.BASE_ASSET_PATH.ONNX_RUNTIME = `${
  path.join(onnxRuntimeWebRoot, "dist")
}/`;
...
export async function transcribePcm16k(audio: Float32Array): Promise<string> {
  ...
  try {
    ...
    console.warn(
      "[voiceTranscribe] Falling back to max-length segmentation.",
      err,
    );
